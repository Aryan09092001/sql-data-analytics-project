...
MySQL vs SQL Server differences handled:
    - IF OBJECT_ID + GO        → DROP VIEW IF EXISTS
    - DATEDIFF(MONTH, a, b)    → TIMESTAMPDIFF(MONTH, a, b)
    - GETDATE()                → CURRENT_DATE
    - CAST(x AS FLOAT)         → x * 1.0  (forces decimal arithmetic in MySQL)
===============================================================================
*/
...
product_aggregations AS (
    ...
    SELECT
        ...
        ROUND(AVG(sales_amount * 1.0 / NULLIF(quantity, 0)), 1) AS avg_selling_price
    FROM base_query
    GROUP BY
        product_key,
        product_name,
